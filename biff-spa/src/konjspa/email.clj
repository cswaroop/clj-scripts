...
(def templates
  {:biff.auth/signup
   (fn [{:keys [biff.auth/link]}]
     {:subject "Sign up for konjspa"
      :html (rum/render-static-markup
              [:div
               [:p "We received a request to sign up for konjspa using this email address."]
   ...
   :biff.auth/signin
   (fn [{:keys [biff.auth/link]}]
     {:subject "Log in to konjspa"
      :html (rum/render-static-markup
              [:div
               [:p "We received a request to log in to konjspa using this email address."]
...
; :mailgun/from "konjspa <contact@yourdomain.example.com>"
